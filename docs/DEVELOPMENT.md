...
For Unreal on this Mac from an APFS checkout, set `UE_ROOT=/Volumes/UE_5_8_APFS` in your shell, then:
...
## Mac runner maintenance

The repository-scoped macOS ARM64 runner `odr-mac-mini` was registered on 2026-09-20 and verified online with the `odr-mac` label. Runner 2.337.0 is installed at `/Users/simon/.local/share/odr-actions-runner` on APFS; its downloaded archive matched the SHA-256 recorded in [TOOLS.md](TOOLS.md). It runs as a `launchd` user service under the current `simon` account, with no signing secrets. A dedicated runner account remains preferable before giving other contributors push access. Runner jobs checkout under its APFS `_work` directory, not the external `T9` volume. The workflow restricts Mac execution to same-repository `codex/` pull-request branches or pushes to `main`; only trusted maintainers should be allowed to push those branches. The `main` branch protection rule requires `linux-portable` and `mac-unreal` from up-to-date commits, applies to administrators, requires conversation resolution and disallows force pushes and deletion. Confirm both jobs pass on the pull request before merging.

When a Mac job fails, inspect the workflow log, Unreal's `Saved/Logs`, `build/reports/unreal/index.json`, the packaged smoke log, Xcode selection (`xcode-select -p`) and free disk space. Verify the portable `ue-release` library exists before Unreal Build Tool runs. From the runner directory, `./svc.sh status` checks the user service; its logs are in `/Users/simon/Library/Logs/actions.runner.simonperegrinvs-open-dynamic-rpg.odr-mac-mini/`. `gh api repos/simonperegrinvs/open-dynamic-rpg/actions/runners` checks GitHub's online status and labels. If `._*` files appear in a package or source, move the checkout to APFS and clean generated directories. If automation appears idle after a test, use the semicolon form `Automation RunTests ODR.BlacksmithMine;Quit`; a separate comma-separated console `Quit` can stop or hang at the wrong time. Restart or update the runner service only after draining current jobs. Re-run one trusted-branch workflow to verify any runner or Xcode change.
